...
function DashboardPage() {
  ...
  return (
    <div className="space-y-6 relative">
      {/* Header Panel */}
      <div className="panel p-4 sm:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white border border-ink/5">
        <div className="min-w-0 flex-1">
          <p className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.25em] text-accent">
            {getRoleHeaderLabel()}
          </p>
          <h1 className="text-xl sm:text-3xl font-black text-ink mt-1 break-words">
            Welcome back, {user?.name || "Member"}
          </h1>
          <p className="text-xs text-ink/50 mt-1 flex flex-wrap items-center gap-1">
            Logged in: <b className="capitalize text-accent">{role}</b> • Campus ID: {user?.collegeId || "1015"}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0 self-end sm:self-auto">
          ...
          <button
            ...
            className="p-2.5 sm:p-3 border border-ink/10 rounded-2xl bg-white hover:bg-ink/5 text-ink/75 transition relative"
            title="View Alerts"
          >
          ...
          <button
            onClick={loadDashboard}
            className="p-2.5 sm:p-3 border border-ink/10 rounded-2xl bg-white hover:bg-ink/5 text-ink/75 transition"
            title="Refresh Data"
          >
